...
<script type="text/javascript">
    var app = angular.module('myApp', ['ngRoute', 'ui.bootstrap', 'jcs-autoValidate'])
        .config(['$routeProvider', function ($routeProvider) {
            $routeProvider
                .when('/list', {templateUrl: '<c:url value="/mis/ifmanager/phonepin/list.tpl.jsp"/>'})
                .when('/edit', {templateUrl: '<c:url value="/mis/ifmanager/phonepin/edit.tpl.jsp"/>'})
    ...
    app.controller('listCtrl', function ($scope, $rootScope, $http) {
        ...
        $scope.pageChanged = function () {
            $http.post("<c:url value="/mis/ifmanager/phonepin/list"/> ", $scope.queryParams).success(function (response) {
                $scope.loading = false;
                if (response.success) {
            ...
            bootbox.confirm({
                ...
                buttons: {
                    ...
                    if (result) {
                        ...
                        $http.post("<c:url value="/mis/ifmanager/phonepin/deletes"/>", param).success(function (data) {
                            if (data.success) {
...
//可现实分页item数量
    ...
    app.controller('editCtrl', function ($scope, $http, $routeParams) {
        ...
        $scope.phonepinPo = {};
        $scope.canAdd = PrivilegeService.hasPrivilege('add');
        $http.get("<c:url value="/mis/ifmanager/phonepin/getActionTypeList"/> ").success(function (response) {
          if (response.success){
        ...
        var url = "<c:url value="/mis/ifmanager/phonepin/create"/>";
        if ($routeParams.id != null) {
            url = "<c:url value="/mis/ifmanager/phonepin/get"/>" + "?id=" + $routeParams.id;
        }
        $http.get(url)
        ...
        $scope.save = function () {
            if ($scope.editForm.$valid) {
                var postUrl = '<c:url value="/mis/ifmanager/phonepin/saveForUpdated"/>';
                if ($scope.phonepinPo.id == null) {
                    postUrl = '<c:url value="/mis/ifmanager/phonepin/saveForCreated"/>';
                }

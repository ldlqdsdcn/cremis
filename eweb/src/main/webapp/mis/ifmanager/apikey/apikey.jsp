...
<script type="text/javascript">
    var app = angular.module('myApp', ['ngRoute', 'ui.bootstrap', 'jcs-autoValidate'])
        .config(['$routeProvider', function ($routeProvider) {
            $routeProvider
                .when('/list', {templateUrl: '<c:url value="/mis/ifmanager/apikey/list.tpl.jsp"/>'})
                .when('/edit', {templateUrl: '<c:url value="/mis/ifmanager/apikey/edit.tpl.jsp"/>'})
    ...
    app.controller('listCtrl', function ($scope, $rootScope, $http) {
        ...
        $scope.pageChanged = function () {
            $http.post("<c:url value="/mis/ifmanager/apikey/list"/>", $scope.queryParams)
            ...
            bootbox.confirm({
                ...
                buttons: {
                    ...
                    if (result) {
                        ...
                        $http.post("<c:url value="/mis/ifmanager/apikey/deletes"/>", param).success(function (data) {
                            if (data.success) {
            ...
            bootbox.confirm({
                ...
                buttons: {
                    ...
                    if (result) {
                        ...
                        $scope.queryParams.init = true;
                        var param = {"queryParams": $scope.queryParams, "ids": ids};
                        $http.post("<c:url value="/mis/ifmanager/apikey/logicDelete"/>", param).success(function (data) {
                            if (data.success) {
                                $scope.updateList(data.data);
                                bootbox.alert("<eidea:message key="module.deleted.success"/>");
                            } else {
...
//可现实分页item数量
    ...
    app.controller('editCtrl', function ($scope, $http, $routeParams) {
        ...
        if ($routeParams.id != null) {
            url = "<c:url value="/mis/ifmanager/apikey/get"/>" + "?id=" + $routeParams.id;
        }
        $http.get(url)
            .success(function (response) {
                if (response.success) {

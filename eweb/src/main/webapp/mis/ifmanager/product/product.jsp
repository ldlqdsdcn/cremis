...
<script type="text/javascript">
    var app = angular.module('myApp', ['ngRoute', 'ui.bootstrap', 'jcs-autoValidate'])
        .config(['$routeProvider', function ($routeProvider) {
            $routeProvider
                .when('/list', {templateUrl: '<c:url value="/mis/ifmanager/product/list.tpl.jsp"/>'})
                .when('/edit', {templateUrl: '<c:url value="/mis/ifmanager/product/edit.tpl.jsp"/>'})
    ...
    app.controller('listCtrl', function ($scope, $rootScope, $http) {
        ...
        $scope.pageChanged = function () {
            $http.post("<c:url value="/mis/ifmanager/product/list"/> ", $scope.queryParams).success(function (response) {
                $scope.loading = false;
                if (response.success) {
            ...
            bootbox.confirm({
                ...
                buttons: {
                    ...
                    if (result) {
                        ...
                        $http.post("<c:url value="/mis/ifmanager/product/deletes"/>", param).success(function (data) {
                            if (data.success) {
...
//可现实分页item数量
    ...
    app.controller('editCtrl', function ($scope, $http, $routeParams) {
        ...
        $scope.misProductPo = {};
        $scope.canAdd = PrivilegeService.hasPrivilege('add');
        $http.get("<c:url value="/mis/ifmanager/product/getPlatformTypeList"/> ").success(function (response) {
            if (response.success){
        ...
        var url = "<c:url value="/mis/ifmanager/product/create"/>";
        if ($routeParams.productCode != null) {
            url = "<c:url value="/mis/ifmanager/product/get"/>" + "?productCode=" + $routeParams.productCode;
        }
        $http.get(url)
        ...
        $scope.save = function () {
            ...
            if ($scope.editForm.$valid) {
                var postUrl = '<c:url value="/mis/ifmanager/product/saveForUpdated"/>';
                if ($scope.misProductPo.updateTime == null) {
                    postUrl = '<c:url value="/mis/ifmanager/product/saveForCreated"/>';
                }

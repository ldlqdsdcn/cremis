...
<script type="text/javascript">
    var app = angular.module('myApp', ['ngRoute', 'ui.bootstrap', 'jcs-autoValidate'])
        .config(['$routeProvider', function ($routeProvider) {
            $routeProvider
                .when('/list', {templateUrl: '<c:url value="/mis/ifmanager/userphone/list.tpl.jsp"/>'})
                .when('/edit', {templateUrl: '<c:url value="/mis/ifmanager/userphone/edit.tpl.jsp"/>'})
    ...
    app.controller('listCtrl', function ($scope, $rootScope, $http) {
        ...
        $scope.pageChanged = function () {
            $http.post("<c:url value="/mis/ifmanager/userphone/list"/> ", $scope.queryParams).success(function (response) {
                $scope.loading = false;
                if (response.success) {
            ...
            bootbox.confirm({
                ...
                buttons: {
                    ...
                    if (result) {
                        ...
                        $http.post("<c:url value="/mis/ifmanager/userphone/deletes"/>", param).success(function (data) {
                            if (data.success) {
...
//可现实分页item数量
    ...
    app.controller('editCtrl', function ($scope, $http, $routeParams) {
        ...
        $scope.userphonePo = {};
        $scope.canAdd = PrivilegeService.hasPrivilege('add');
        var url = "<c:url value="/mis/ifmanager/userphone/create"/>";
        if ($routeParams.id != null) {
            url = "<c:url value="/mis/ifmanager/userphone/get"/>" + "?id=" + $routeParams.id;
        }
        $http.get(url)
        ...
        $scope.save = function () {
            if ($scope.editForm.$valid) {
                var postUrl = '<c:url value="/mis/ifmanager/userphone/saveForUpdated"/>';
                if ($routeParams.id == null) {
                    postUrl = '<c:url value="/mis/ifmanager/userphone/saveForCreated"/>';
                }

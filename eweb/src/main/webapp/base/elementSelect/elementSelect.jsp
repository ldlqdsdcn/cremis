...
<script type="text/javascript">
    var app = angular.module('myApp', ['ngFileUpload','ngRoute', 'ui.bootstrap', 'jcs-autoValidate'])
            .config(['$routeProvider', function ($routeProvider) {
                $routeProvider
                        .when('/list', {templateUrl: '<c:url value="/base/elementSelect/list.tpl.jsp"/>'})
                        .when('/edit', {templateUrl: '<c:url value="/base/elementSelect/edit.tpl.jsp"/>'})
    ...
    app.controller('listCtrl', function ($rootScope,$scope,$http,$window) {
        ...
        $scope.pageChanged = function () {
            $http.post("<c:url value="/base/elementSelect/list"/>", $scope.queryParams)
...
//可现实分页item数量
    ...
    app.controller('editCtrl', function ($routeParams,$scope, $http,$window,$timeout, Upload) {
        ...
        if ($routeParams.id != null) {
            url = "<c:url value="/base/elementSelect/get"/>" + "?id=" + $routeParams.id;
        }
        $http.get(url)
                .success(function (response) {
                    if (response.success) {

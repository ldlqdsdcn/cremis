    ...
    app.controller('editCtrl', function ($scope, $http, $routeParams) {
        ...
        $scope.save = function () {
            if ($scope.editForm.$valid) {
                ...
                if ($scope.apiKeyPo.id == null) {
                    postUrl = '<c:url value="/base/apikey/saveForCreated"/>';
                }
                $http.post(postUrl, $scope.apiKeyPo).success(function (data) {
                    if (data.success) {
                        $scope.message = "<eidea:label key="base.save.success"/>";
                        $scope.apiKeyPo = data.data;
                    }
                    else {
        ...
        $scope.create = function () {
            ...
            $http.get(url)
                .success(function (response) {
                    if (response.success) {
                        $scope.apiKeyPo = response.data;
                        $scope.canSave = (PrivilegeService.hasPrivilege('add') && $scope.apiKeyPo.id == null) || PrivilegeService.hasPrivilege('update');
                    }
                    else {
                        bootbox.alert(response.message);
                    }

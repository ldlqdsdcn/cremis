...
<script type="text/javascript">
    var app = angular.module('myApp', ['ngRoute', 'ui.bootstrap', 'jcs-autoValidate'])
        .config(['$routeProvider', function ($routeProvider) {
            $routeProvider
                .when('/list', {templateUrl: '<c:url value="/base/bills/list.tpl.jsp"/>'})
                .when('/edit', {templateUrl: '<c:url value="/base/area/edit.tpl.jsp"/>'})
    ...
    app.controller('listCtrl', function ($scope,$rootScope, $http) {
        ...
        $scope.pageChanged = function () {
            $http.post("<c:url value="/base/bills/list"/>", $scope.queryParams)
...
//可现实分页item数量
    ...
    <%--app.controller('editCtrl', function ($scope, $http, $routeParams) {--%>
        ...
        <%--$scope.areaPo = {};--%>
        <%--$scope.canAdd=PrivilegeService.hasPrivilege('add');--%>
        <%--var url = "<c:url value="/base/area/create"/>";--%>
        <%--if ($routeParams.id != null) {--%>
            <%--url = "<c:url value="/base/area/get"/>" + "?id=" + $routeParams.id;--%>
        <%--}--%>
        <%--$http.get(url)--%>
            <%--.success(function (response) {--%>
                <%--if (response.success) {--%>
                    <%--$scope.areaPo = response.data;--%>
        ...
        <%--$scope.save = function () {--%>
            <%--if ($scope.editForm.$valid) {--%>
                <%--var postUrl = '<c:url value="/base/area/saveForUpdated"/>';--%>
                <%--if ($scope.areaPo.id == null) {--%>
                    <%--postUrl = '<c:url value="/base/area/saveForCreated"/>';--%>
                <%--}--%>
                <%--$http.post(postUrl, $scope.areaPo).success(function (data) {--%>
                    <%--if (data.success) {--%>
                        <%--$scope.message = "<eidea:label key="base.save.success"/>";--%>
                        <%--$scope.areaPo = data.data;--%>
                    <%--}--%>
                    <%--else {--%>

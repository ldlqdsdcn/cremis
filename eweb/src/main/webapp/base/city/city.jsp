...
<html>
<head>
    <title><%--城市--%><eidea:label key="city.title"/></title>
...
<script type="text/javascript">
    var app = angular.module('myApp', ['ngRoute', 'ui.bootstrap', 'jcs-autoValidate'])
            .config(['$routeProvider', function ($routeProvider) {
                $routeProvider
                        .when('/list', {templateUrl: '<c:url value="/base/city/list.tpl.jsp"/>'})
                        .when('/edit', {templateUrl: '<c:url value="/base/city/edit.tpl.jsp"/>'})
...
$http.post("<c:url value="/base/city/list"/>", $scope.queryParams)
...
$http.post("<c:url value="/base/city/deletes"/>", param).success(function (data) {
if (data.success) {
...
$scope.pageChanged();
    ...
    app.controller('editCtrl', function ($scope, $http, $routeParams) {
        ...
        $scope.cityPo = {};
        $scope.canAdd=PrivilegeService.hasPrivilege('add');
        var url = "<c:url value="/base/city/create"/>";
        if ($routeParams.id != null) {
            url = "<c:url value="/base/city/get"/>" + "?id=" + $routeParams.id;
        }
        $http.get(url)
                .success(function (response) {
                    if (response.success) {
                        $scope.cityPo = response.data;
                        $scope.canSave=(PrivilegeService.hasPrivilege('add')&&$scope.cityPo.id==null)||PrivilegeService.hasPrivilege('update');
                    }
                    else {
                        bootbox.alert(response.message);
                    }
        ...
        $scope.save = function () {
            if ($scope.editForm.$valid) {
                var postUrl = '<c:url value="/base/city/saveForUpdated"/>';
                if ($scope.cityPo.id == null) {
                    postUrl = '<c:url value="/base/city/saveForCreated"/>';
                }
                $http.post(postUrl, $scope.cityPo).success(function (data) {
                    if (data.success) {
                        $scope.message = "<eidea:label key="base.save.success"/>";
                        $scope.cityPo = data.data;
                    }
                    else {
